...
### 1.4 新功能开发流程（toAdd工作区）
1. **开发阶段**：在 `toAdd/` 目录下创建功能测试子目录（如 `toAdd/planner/`）
2. **独立测试**：在隔离环境中测试新功能，避免影响主项目
3. **代码迁移**：测试通过后，将代码迁移到正式目录：
   - 后端代码 → 合并到 `app.py` 或相应后端模块
   - HTML模板 → 移动到 `templates/` 目录
   - JavaScript → 移动到 `static/js/modules/` 目录
   - CSS → 移动到 `static/css/` 目录
4. **集成测试**：在正式环境中进行集成测试
5. **清理工作区**：迁移完成后，删除 `toAdd/` 中的测试代码
